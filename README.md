...
The pSF-Toolbox ([now published in JORS](https://doi.org/10.5334/jors.610)) streamlines the population spatial frequency tuning (pSFT) approach, originally developed by [Aghajari, Vinke, &amp; Ling 2020 (Journal of Neurophysiology)](https://doi.org/10.1152/jn.00291.2019), with accessibility in mind.
...
- `/stimuli`: Stimulus textures will be stored here by default.
  - `verifyStimuli`: Analyzes spectral energy of stimuli. Can generate and save experimental stimuli as well. The script can also save energy profiles to a `.mat` file. The saved energy file contains energy profiles, frequency bin centers, target and peak frequencies, and stimulus parameters for all textures.
...
![Figure 2](figures/pSFT_stimuli.png)

**Figure 2: Stimuli.** (a) Stimulus generation. Every stimulus presented begins as a random sample of uniform white noise. A 2D fast Fourier transform (FFT) is performed on the noise sample, which is then masked by a circularly symmetric narrow-band filter (filter width is exaggerated here for visualization purposes). The resulting image is returned to the spatial domain and converted to 8-bit grayscale. (b) Examples of bandpass-filtered stimuli. (c) Examples of stimulus SF energy as a function of SF. The target SF (cpd) is provided above the peak of each energy profile. 
...
![Figure 3](figures/pSFT_pipeline.png)
