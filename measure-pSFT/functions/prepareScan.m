...
%   Opens window, defines parameters, device input, loads/creates textures, generates frames, and makes stimuli for drawing.
%
% Syntax
...
% macOS Retina displays scale the window 2x — recalculate dimensions
if toggles.macOS
    windowRect = Screen('Rect', w.window);
    w.screen_width_px = windowRect(3);
    w.screen_height_px = windowRect(4);
    w.centerX = round(w.screen_width_px / 2);
    w.centerY = round(w.screen_height_px / 2);
    w.ppd = round(w.screen_width_px / w.visual_angle);
end
...
texture_filepath = fullfile(dirs.stimuli_dir, ...
    sprintf('bandpass_filtered_noise_%s_%s_%s_%s_%s.mat', ...
    strrep(sprintf('%0.0f', w.ppd), '.', ''), ...
    strrep(sprintf('%0.0f', p.stimulus_radius_px), '.', ''), ...
...
if exist(texture_filepath, 'file') == 2
    ...
    [textures, filters] = createTextures(p.stimulus_radius_px, p.stimulus_contrast, p.noise_filter_count, p.noise_sample_count, w.ppd, toggles.save_textures, texture_filepath);
end

stimuli.textures = textures;
...
stimuli.textures_made = nan(p.noise_filter_count, p.noise_sample_count);
for n_filter = 1:p.noise_filter_count
    for n_sample = 1:p.noise_sample_count
        stimuli.textures_made(n_filter, n_sample) = Screen('MakeTexture', w.window, stimuli.textures(:, :, n_filter, n_sample));
    end
end

%%  Patches

stimuli.stimuli_patch = CenterRectOnPoint([0 0 w.screen_height_px w.screen_height_px], w.centerX, w.centerY);
