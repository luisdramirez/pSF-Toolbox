...
%
%   Output Arguments
%       chunk – structure with updated fields:
...
function chunk = fitVoxels(chunk, I, p, toggles)
...
for vox = 1:num_vox
    ...
    curr_fixed_params_vox = {I, chunk.measured_BOLD(:,vox), chunk.HRF(:,vox)};
    ...
    tmp_start_sse(vox) = evaluatePSFT(pSFT_start_vals, curr_fixed_params_vox);

    %% fmincon to estimate pSFT

    pSFT_model = @(pSFT_free_params) evaluatePSFT(pSFT_free_params, curr_fixed_params_vox);
    ...
    est_BOLD = generateBOLD(R, curr_fixed_params_vox{3}, param_est(3), param_est(4));
    tmp_est_BOLD(:,vox) = est_BOLD;

    % Calculate R^2
    tmp_r2(vox) = R2(curr_fixed_params_vox{2}, est_BOLD);
