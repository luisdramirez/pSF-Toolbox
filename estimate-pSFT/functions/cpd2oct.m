...
% cpd2oct - Convert cycles per degree (cpd) to octaves
%   Finds the lowest and highest SF in cycles/degree (cpd) that produced half the maximum response.
%   Converts cpd to octaves.
%
%   Syntax
%       [bandwidth_oct, sf_halfmax] = cpd2oct(pSFT, sf)
%
%   Input Arguments
%       pSFT – pSFT curve
...
%
%   Output Arguments
%       bandwidth_oct – bandwidth in octaves
%       sf_halfmax – spatial frequencies that produced half the maximum response
...
function [bandwidth_oct, sf_halfmax] = cpd2oct(pSFT, sf)
    ...
    %% Plot pSFT and bandwidth (optional)

    %{

    figure
    plot(sf, pSFT); hold on;
    line([sf_halfmax(1) sf_halfmax(1)],[0 max_pSFT],'Color',[0 1 0],'LineStyle','--');
    line([sf_halfmax(2) sf_halfmax(2)],[0 max_pSFT],'Color',[0 1 0],'LineStyle','-');
    set(gca,'XScale','log');
